...
namespace mrf_uai_input {
   ...
   template<> struct action< allocate_data > {
      ...
      static void apply(const Input& in, mrf_input& input, mrf_input_helper& input_helper) 
      {
          ...
          // actually we must write 0 into unaries, since not all unary potentials must be present
   ...
   mrf_input parse_file(const std::string& filename)
   {
       ...
       const bool read_success = problem.parse< grammar, action >( input, input_helper );
       assert(read_success);
   ...
   mrf_input parse_string(const std::string& uai_string)
   {
       mrf_input input;
       mrf_input_helper input_helper;
       const bool read_success = pegtl::parse<grammar, action>(uai_string,"",input, input_helper);
       assert(read_success);
       return input;
   }
